...
abstract class Process
{
    ...
    public function pipeCreate() {
        if(! posix_access($this->pipeFile, POSIX_F_OK) ) {
    ...
    public function pipeWrite($signal = '', $pipeFile = '') {
        ...
        if(! posix_access($pipeFile, POSIX_F_OK | POSIX_W_OK)) return false;
    ...
    public function pipeRead(Closure $closure = null) {
        // check pipe
        if(! posix_access($this->pipeFile, POSIX_F_OK | POSIX_R_OK)) return false;
        // open pipe
        do {
